...
// 4096.0
#define NOISE_SPEED_X 10000.0
// 8192.0
#define NOISE_SPEED_Y 10000.0
...
// Version that preserves color range [0,1]
vec3 adjustContrast(vec3 color, float contrast) {
    vec3 result = (color - 0.5) * contrast + 0.5;
    return clamp(result, 0.0, 1.0);
}

// Brightness with clamping to prevent overflow
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float noiseContent = smoothstep(0.4, 0.6, fract(sin(uv.x * uv.y * (1.0-uv.x) * (1.0-uv.y) * (iTime*0.0+1.0) * NOISE_SPEED_X) * 65536.0));
    float noiseUniform = smoothstep(0.4, 0.6, fract(sin(uv.x * uv.y * (1.0-uv.x) * (1.0-uv.y) * (iTime*0.0+1.0) * NOISE_SPEED_Y) * 65536.0));
